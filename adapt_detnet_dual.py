...
def parse_the_args():
    ...
    parser.add_argument("-trs", "--datasets_train", nargs="+", default='ah', type=str,
                        help="sub datasets, should be set as ah")
    parser.add_argument("-tes", "--datasets_test", nargs="+", default="ah", type=str,
                        help="sub datasets, should be set as ah")
    # Miscs
    ...
    parser.add_argument('--set', type=str, default='val', help='which set to adapt to, val or test')
...
def main(args):
    ...
    if args.evaluate:
        for key, value in test_loader_dic.items():
            validate(value, model, criterion, key, R=R_mom, epoch=None, args=args)
        return
    ...
    for epoch in range(args.start_epoch, args.start_epoch + args.epochs):
        ...
        for key, value in test_loader_dic.items():
            auc[key], acc_hm[key] = validate(value, model, criterion, key, R=R_mom, epoch=epoch, args=args,
                                             write_epoch=epoch)
...
def validate(val_loader, model, criterion, key, epoch, args, R, stop=-1, write_epoch=None):
    ...
    with torch.no_grad():
        for i, (metas1, metas2) in tqdm(enumerate(val_loader)):
            ...
            pred_joint_align1, pred_joint_align2 = func.merge_from_2hands(pred_joint_align1, pred_joint_align2,
                                                                          valid1, valid2, R12=R, R21=R.T)
...
def train(train_loader, model, model_ema, criterion, optimizer, args, loss_all, epoch=0, R12=None):
    ...
    for i, (metas1, metas2) in enumerate(train_loader):
        ...
        # TODO: improve the adaptation algorithm
        ...
        # TODO: improve the loss functions
